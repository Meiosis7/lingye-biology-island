...
const files=['index.html','src/style.css','src/data.js','src/core.js','src/game.js','assets/lingye/world.png','assets/lingye/creatures.png','assets/lingye/researcher-sheet.png'];
...
for(const image of ['creatures.png','researcher-sheet.png'])css=css.replaceAll(`url('../assets/lingye/${image}')`,`url('${uri('assets/lingye/'+image)}')`);
html=html.replace('<link rel="stylesheet" href="./src/style.css?v=2.1.0">','<style>'+css+'</style>');
for(const file of ['src/data.js','src/core.js','src/game.js'])html=html.replace(`<script src="./${file}?v=2.1.0"></script>`,'<script>'+fs.readFileSync(path.join(root,file),'utf8').replace(/<\/script/gi,'<\\/script')+'</script>');
for(const file of ['assets/lingye/world.png'])html=html.replace(`src="./${file}"`,`src="${uri(file)}"`);
if(/<(?:script|link|img)[^>]+(?:src|href)="\.\//.test(html)||/url\(['"]?\.\.\//.test(html))throw Error('Offline dependency remains');
