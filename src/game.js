...
function travel(id){if(moving||!rBy(id))return;if(id===save.region){renderRegion();return}const facing=Core.facingBetween(rBy(save.region),rBy(id),$('hero').dataset.facing);$('hero').dataset.facing=facing;save.region=id;moving=true;$('hero').classList.add('walking');renderPins();renderRegion();persist();sound();setTimeout(()=>{moving=false;$('hero').classList.remove('walking');renderRegion()},1000)}
...
function showResearcher(){
 if(moving)return;
 openModal(title('EXPLORER PROFILE · 01','岑叶 · 野外研究员')+`<div class="researcher-profile"><div class="researcher-portrait"><span class="researcher-sprite" role="img" aria-label="岑叶的完整像素立绘"></span><p>栖光研究所 / 见习自然研究员</p><div class="researcher-directions">${[['正面','0%'],['左侧','66.666667%'],['右侧','100%'],['背面','33.333333%']].map(([name,row])=>`<div><span class="researcher-sprite" style="--hero-col:${row}" role="img" aria-label="${name}形象"></span><small>${name}</small></div>`).join('')}</div></div><div class="researcher-story"><p class="eyebrow">CEN YE · FIELD NATURALIST</p><h3>把好奇心，装进背包。</h3><p>岑，取自有林木的小山；叶，是认识植物时最容易遇见的线索。岑叶相信，辨认生命要从耐心观察开始。</p><dl><dt>叶徽探险帽</dt><dd>米白帽沿遮挡阳光，叶片徽记呼应栖光研究所。</dd><dt>浅色野外外套</dt><dd>浅色外套叠穿绿色内衫，深色短裤和耐磨靴让林地中的轮廓清楚可辨。</dd><dt>采样背包与记录袋</dt><dd>背包上的叶片标记呼应探险帽，宽肩带与分层收纳方便野外观察。先记录，尽量不惊扰原有栖息地。</dd></dl><div class="note-box">在地图上前往不同地点，岑叶会转向并迈步行走。点击人物可随时查看档案。</div></div></div>`);
}
$('researcher-btn').onclick=showResearcher;
